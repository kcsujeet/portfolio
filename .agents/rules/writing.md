...
- Derive factors, do not assert them. "So table size matters" out of nowhere reads as a leap. Either derive it from the mechanism just explained (a rewrite visits every row, so more rows means more time), or open with the frame ("comes down to three things stacking") and take the factors in turn.
- A pivot phrase at the start of a paragraph ("Stepping back", "Put differently") is usually a heading trying to happen. If the question the text answers has changed, start a new section instead.
...
- Give every named variant the family treatment. If the post names a specific kind of thing (a GIN index, an `ACCESS EXCLUSIVE` lock), briefly lay out the family: what the default is, what the alternatives are, why this one applies here, with a docs link for the full list. The lock-modes list and the index-types passage in the locking post are the pattern.
- Bracket an aside as an aside, at both ends. Do not open scene-setting with a teaser; "It also created an index, and not the usual kind" promises a payoff that never arrives. Frame it as answering the reader's question instead: "You might be wondering about the `using: :gin` in that line, so a quick word on index types first." And when the detail plays no further part, return from the aside out loud and release the reader: "Okay, back to the story. The index type won't matter again; what does matter is that the migration builds an index on a busy table."
- After quoting docs that lean on a term of art ("volatile"), define the term in the next paragraph: what it means, one example that qualifies and one that doesn't, then the mechanism it drives.
- When two true statements read as a contradiction, reconcile them on the page. Saying `UPDATE` takes a table-level `ROW EXCLUSIVE` lock and, a section later, that Postgres locks just the row is a contradiction to the reader until you say the table-level mode doesn't conflict with itself.
...
- Admit what you did not know, and hedge honestly rather than projecting certainty. "I knew this meant `useWatch` gives better performance but I wasn't sure in what way or by how much." "I'd be lying if I said it was all smooth." "I can't prove that's exactly what happened, because I never traced it down to a specific run or retry." Hedge in plain words; idiom pairs like "the shape fits, and the fix is the tell" are doing a voice.
...
- State the conclusion before the evidence. Lead with the plain fact ("While a plain `CREATE INDEX` runs, reads keep working but writes wait"), then let the docs quote or link back it up. A paragraph that opens on a quote and chains lock-mode names to reach its point reads as heavy.
- The fix is the payoff of an incident post; give it its own beat. Never let it debut as a clause inside a hedge or an inference ("the fix is the tell: the follow-up PR added..."). Reveal it as an action ("I reverted the deploy and shipped a follow-up PR with one change"), then explain what the option does and which part of the fix addressed which symptom.
- Keep a hedge and a reveal in separate paragraphs; they are different jobs. A hedge lands strongest as what you could see beside what you cannot prove: "I can't prove that's exactly what happened; I never traced it down to a specific run or retry. What I could see was the state itself."
