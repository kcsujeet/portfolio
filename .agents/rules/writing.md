...
- Bracket an aside as an aside, at both ends. Do not open scene-setting with a teaser; "It also created an index, and not the usual kind" promises a payoff that never arrives. Frame it as answering the reader's question instead: "You might be wondering about the `using: :gin` in that line, so a quick word on index types first." And when the detail plays no further part, say so and release the reader: "it's also the last time the index type matters. Everything from here applies to building any index on a busy table."
